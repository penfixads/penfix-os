...
export default async function DailySalesSummaryPage() {
  ...
  const [{ data: payments }, { data: expenses }, { data: summary }, { count: joCount }] = await Promise.all([
    // Today's payments
    supabase
      .from('payments')
    ...
    supabase.from('daily_sales_summary').select('*').eq('summary_date', today).maybeSingle(),
    // Today's JOs count
    supabase
      .from('job_orders')
