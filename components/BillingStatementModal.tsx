...
// A running "here's everything you still owe" statement for a "For Billing" (credit-line)
// client — every job order with an open balance, not just one, so Treasury doesn't have to
// send separate receipts per JO for clients billed periodically instead of per order.
export default function BillingStatementModal({ clientId, clientName, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    ]).then(([{ data: jobsData, error: jobsErr }, { data: clientRow, error: clientErr }]) => {
      ...
      <div className="pf-modal-card pf-modal-wine" style={{ maxWidth: 480, marginTop: '1rem' }}>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1rem' }}>
            {jobs.map(jo => (
              <div key={jo.job_order_id} style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8, padding: '0.6rem 0.75rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
